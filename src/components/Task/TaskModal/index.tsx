...
/**
 * TaskModal组件的属性接口
 * 使用TaskData类型进行数据传输，区别于Task类型：
 * - TaskData: 用于表单数据和API传输，包含parent_id、actions为string[]
 * - Task: 用于应用内部状态，包含children、actions为Action[]
 */
...
/**
 * 任务表单数据接口
 * 用于TaskModal组件内部的表单状态管理
 * 与TaskData的区别：
 * - due_to/reminder: 使用string类型便于datetime-local输入
 * - reminderOffset: 额外字段用于计算提醒时间
 * - actions: 使用Action[]类型便于表单操作
 */
...
/**
 * 提醒时间偏移量选项配置
 * offset单位为毫秒，用于计算相对于due_to的提醒时间
 */
...
export default function TaskModal({ isOpen, onClose, onSave, task, parentTask }: TaskModalProps) {
   ...
   * 根据截止时间和提醒时间计算偏移量选项
   * @param dueDate 截止时间
   * @param reminderDate 提醒时间
   * @returns 匹配的偏移量选项值，默认为'ontime'
   */
  ...
  /**
   * 根据截止时间和偏移量选项计算提醒时间
   * @param dueToStr 截止时间字符串（datetime-local格式）
   * @param offsetValue 偏移量选项值
   * @returns 计算后的提醒时间字符串（datetime-local格式）
   */
...
//   // 使用useState中定义的setFormData函数更新表单数据
    ...
    <div className="task-modal" onClick={(e) => e.stopPropagation()}>
      ...
      <form onSubmit={handleSubmit} className="task-modal-form">
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            <label htmlFor="reminder">计算后的提醒时间</label>
            <input
              id="reminder"
              type="datetime-local"
              value={formData.reminder}
              onChange={(e) => handleInputChange('reminder', e.target.value)}
              className="reminder-time-display"
              title="此时间根据截止时间和提醒偏移量自动计算"
            />
          </div>
        </div>
