...
pub fn enable_auto_launch(enable: bool) -> Result<()> {
    if enable {
        create_shortcut()?;
    }else{
        remove_shortcut()?;
...
pub fn create_shortcut() -> Result<()> {
    ...
    let old_shortcut_path = startup_dir.join("Ducker.lnk");
    let new_shortcut_path = startup_dir.join("Ducker.lnk");
...
pub fn remove_shortcut() -> Result<()> {
    ...
    // 删除旧的快捷方式
    if is_shortcut_enabled().unwrap_or(false) {
