use super::{
    config::WindowConfig,
    schema::{WindowOperationResult, WindowState},
};
use crate::{
    core::{handle, tray},
    logging,
...
use tauri::{Emitter, Listener, Manager, PhysicalPosition, WebviewWindow, Wry};
        ...
        if let Some(existing_window) = self.get_window(window_type) {
            ...
            if let Some(u) = url {
                if let Ok(current_url) = existing_window.url() {
                    ...
                    if parsed_url != u {
                        let _ = existing_window.emit_to( window_type.label(), "renavigate", u);
                    }
                }
            }
